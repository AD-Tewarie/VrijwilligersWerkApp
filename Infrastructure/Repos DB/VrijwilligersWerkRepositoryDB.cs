...
public class VrijwilligersWerkRepositoryDB : IVrijwilligersWerkRepository
{
    ...
    public int AddVrijwilligersWerk(VrijwilligersWerk werk)
    {
        ...
        var command = databaseService.CreateCommand(connection,
            @"INSERT INTO volenteer_work (title, description, max_volenteers, location, reg_count)
            VALUES (
        ...
        command.AddParameter("@reg_count", 0);
    ...
    public bool BewerkAantalRegistraties(int werkId, int wijziging)
    {
        ...
        command.AddParameter("@wijziging", wijziging);
